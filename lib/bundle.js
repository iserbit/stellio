...
var baseRandom = __webpack_require__(51),
    ...
    toFinite = __webpack_require__(105);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_lodash_merge__ = __webpack_require__(103);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_lodash_flatten__ = __webpack_require__(99);
...
class Board {
  constructor(stage, queue, scoreStage) {
    ...
    this.scoreValue = new createjs.Text(this.score, '', '#fe8900');
  ...
  createScore() {
    this.scoreValue.x = 83;
    this.scoreValue.y = 0;
    this.scoreValue.textAlign = 'center';
    this.scoreValue.font = 'bold 45px Open Sans';
    this.scoreStage.addChild(this.scoreValue);
  }
  ...
  moveTile(tile) {
    return event => {
      ...
      const diffRowCol = Math.abs(row) - Math.abs(col);
      const diffColRow = Math.abs(col) - Math.abs(row);
      ...
      //   }
      //
      //   if (col < limitColBot && col > limitColTop){
  ...
  findSwapTgt(tile) {
    return event => {
      ...
      if (row_diff >  10 && row_diff < -10 &&
          col_diff >  10 && col_diff < -10) {
        event.currentTarget.x = tile.col_coord;
        event.currentTarget.y = tile.row_coord;
        this.render();
        return;
  ...
  swapTiles(src, tgt, matches) {
    this.setScore(matches.length + 1);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  queue.on('complete', () => {
    board = new __WEBPACK_IMPORTED_MODULE_3__board_js__["a" /* default */](stage, queue, scoreStage);
    board.render();
  }, this);
});
